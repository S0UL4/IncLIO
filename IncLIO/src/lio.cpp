...
void LIO::Undistort() {
    ...
    std::for_each(std::execution::par_unseq, cloud->points.begin(), cloud->points.end(), [&](auto& pt) {
        ...
        Vec3d pi = ToVec3d(pt);
        Vec3d p_compensate = TIL.inverse() * T_end.inverse() * Ti * TIL * pi;
...
void LIO::Align() {
    ...
    for (const auto& pt : scan_imu_frame->points) {
        PointType p;
    ...
    voxel.setLeafSize(config_.scan_voxel_size, config_.scan_voxel_size, config_.scan_voxel_size);
    voxel.setInputCloud(current_scan_);

    CloudPtr current_scan_filtered(new PointCloudType);
    voxel.filter(*current_scan_filtered);
