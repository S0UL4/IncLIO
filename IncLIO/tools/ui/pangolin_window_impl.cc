...
bool PangolinWindowImpl::Init() {
    ...
    traj_gps_ui_.reset(new ui::UiTrajectory(Vec3f(1.0, 1.0, 51.0 / 255.0)));  // Yellow
...
bool PangolinWindowImpl::UpdateGps() {
    ...
    // Update localization trajectory
...
void PangolinWindowImpl::RenderLabels() {
    // Localization status label, displayed in 3D window
...
void PangolinWindowImpl::Render() {
    ...
    while (!pangolin::ShouldQuit() && !exit_flag_.load()) {
        // Clear entire screen
...
void PangolinWindowImpl::AllocateBuffer() {
    std::string global_text("Welcome to IncLIO.UI");
