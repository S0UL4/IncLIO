#ifndef INCLIO_MATH_UTILS_HPP
#define INCLIO_MATH_UTILS_HPP
 ...
 * @tparam Getter   getter function, takes a container element and returns a D type
 */
...
/**
 * Calculate the mean and covariance of data in a container
 * @tparam C    container type
 * @tparam int 　data dimension
 * @tparam Getter   getter function, takes a container element and returns a Eigen::Matrix<double, dim,1> vector type
 */
...
/**
 * Calculate the mean and covariance of two Gaussian distributions
 * @tparam S    scalar type
 * @tparam D    dimension
 * @param hist_m        history point count
 * @param curr_n        current point count
 * @param hist_mean     history mean
 * @param hist_var      history variance
...
/// xt: It has been verified that this conversion is consistent with the result of tf::createQuaternionFromRPY in ROS
template <typename T>
...
/**
 * Estimate plane: solve plane equation using colPivHouseholderQr, use dynamic matrix to store point cloud coordinates
 */
template <typename T>
inline bool esti_plane_dynamic(Eigen::Matrix<T, 4, 1>& abcd, const std::vector<Vec3d>& point, const double& threshold) {
    if (point.size() < 3) {
        INCLIO_ERROR("the number of points should not be less than 3, given {}", point.size());
        return false;
    }
    // Don't change the type of matrix A and b (including double and Eigen::Dynamic), it may cause problems when points.size() is 3 or 4, the reason is unknown.
...
/**
 * Calculate the mean and covariance of two Gaussian distributions incrementally
 * @see https://www.cnblogs.com/yoyaprogrammer/p/delta_variance.html
 * @param hist_n     history data count
 * @param hist_mean history mean
 * @param hist_var2 history variance
 * @param curr_n    current data count
 * @param curr_mean current mean
...
/**
 * Pose interpolation algorithm for data with timestamp, such as IMU and odometry data
 * @tparam T    data type
 * @tparam C    data container type
 * @tparam FT get time function, takes a data element and returns its timestamp
 * @tparam FP get pose function
 * @param query_time query time
 * @param data  data container
 * @param take_pose_func predicate to extract pose from data, takes a data element and returns an SE3
 * @param result query result
 * @param best_match_iter the closest match found
 *
 * NOTE : query_time must be between the max and min time of data (allowing 0.5s error)
 * data's map is sorted by time
 * @return
 */
...
inline bool PoseInterp(double query_time, C&& data, FT&& take_time_func, FP&& take_pose_func, SE3& result,
    ...
    if (data.empty()) {
        INCLIO_INFO("cannot interp because data is empty. ");
        return false;
    }
    ...
    double s = (query_time - take_time_func(*match_iter)) / dt;  //  when s=0, it's the first frame, when s=1, it's the next frame
    //  there is a bug that dt is 0, which may be caused by duplicate timestamps in the data, we need to check if dt is 0 before interpolation
    if (fabs(dt) < 1e-6) {
...
/**
 * Marginalization
 * @param H the information matrix to be marginalized, which is a square matrix, and the block to be marginalized is a square block on the diagonal
 * @param start the start index of the block to be marginalized, inclusive
 * @param end the end index of the block to be marginalized, inclusive
 * @return
 */
 ...
 * @tparam T    data type 
 * @param query_time query time
 * @param data  data container, map with time as key and data as value
 * @param take_pose_func  from data to extract pose predicate
 * @param result query result
 * @param best_match_iter the closest match found
 *
 * NOTE query_time must be between the max and min time of data, no extrapolation
 * data's map is sorted by time
 * @return whether the interpolation is successful
 */
template <typename T>
bool PoseInterp(double query_time, const std::map<double, T>& data, const std::function<SE3(const T&)>& take_pose_func,
    ...
    if (data.empty()) {
        INCLIO_INFO("data is empty");
        return false;
    }

    if (query_time > data.rbegin()->first) {
        INCLIO_INFO("query time is later than last, but within acceptable range, query: {:.18f}, end time: {:.18f}",
                    query_time, data.rbegin()->first);
